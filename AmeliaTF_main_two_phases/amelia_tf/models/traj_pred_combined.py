...
class ModePredictionModel(LightningModule):
    ...
    def __init__(
        ...
        self.use_mode_weights = bool(
            getattr(extra_params, "use_mode_weights", True)
        )
        # Historical mode-weighted cross entropy settings.
        self.use_smoothing_with_hard_mask = bool(
            getattr(extra_params, "use_smoothing_with_hard_mask", False)
        )

    def setup(self, stage: str) -> None:
        """Inject class weights when enabled for imbalanced mode classification."""
        if stage == "fit":
            if not self.use_mode_weights:
                print("Mode class weights disabled")
            else:
            ...
            if self.use_smoothing_with_hard_mask:
                print("Label smoothing enabled with hard feasibility mask")
    ...
    def model_step(
        ...
        if Y_mode is not None:
            ...
            use_smoothing = (
                0.1 if (
                    not getattr(self.net, 'apply_hard_mask', False)
                    or self.use_smoothing_with_hard_mask
                )
                else 0.0
            )
            loss = F.cross_entropy(
                ...
                weight=self.mode_weights if self.use_mode_weights else None,
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> None:
        ...
        if not getattr(self, "_running_initial_validation", False):
            self.val_loss(loss)
    ...
    def _log_validation_metrics(
        ...
        metric_values = {
            f"{prefix}/modal_accuracy": modal_metrics['accuracy'],
            f"{prefix}/modal_macro_f1": modal_metrics['macro_f1'],
            f"{prefix}/modal_weighted_f1": modal_metrics['weighted_f1'],
        }

        for i, mode_idx in enumerate(self.valid_modes):
            mode_name = self.mode_names[mode_idx]
            metric_values.update({
                f"{prefix}/modal_precision/{mode_name}":
                    modal_metrics['precision_per_mode'][mode_idx],
                f"{prefix}/modal_recall/{mode_name}":
                    modal_metrics['recall_per_mode'][mode_idx],
                f"{prefix}/modal_f1/{mode_name}":
                    modal_metrics['f1_per_mode'][mode_idx],
                f"{prefix}/modal_support/{mode_name}":
                    modal_metrics['support_per_mode'][mode_idx],
            })

        if prefix == "val_initial":
            # self.log() is not allowed from on_fit_start; write the initial
            # full-validation metrics directly to the experiment logger.
            ...
            for name, value in metric_values.items():
                self.log(name, value, on_epoch=True,
                         prog_bar=name == f"{prefix}/modal_accuracy")
    ...
    def on_fit_start(self) -> None:
        """Evaluate the untrained model on the complete validation set."""
        ...
        if val_loaders is None:
            ...
            if datamodule is None or not hasattr(datamodule, "val_dataloader"):
                raise RuntimeError(
                    "Initial validation requires a datamodule with val_dataloader()")
            val_loaders = datamodule.val_dataloader()

        if val_loaders is None:
            raise RuntimeError("val_dataloader() returned None during initial validation")

        if isinstance(val_loaders, (list, tuple)):
            if len(val_loaders) != 1:
                raise RuntimeError(
                    "Initial validation requires exactly one validation dataloader")
            val_loader = val_loaders[0]
        else:
            val_loader = val_loaders

        was_training = self.training
        self._running_initial_validation = True
        self.eval()
        try:
            ...
            self._running_initial_validation = False
            self.val_loss.reset()
            if was_training:
                self.train()
        ...
        self._log_validation_metrics(
            prefix="val_initial",
            title="Initial Validation (Before Training)")

    def on_validation_epoch_end(self) -> None:
        # Sanity-check batches are intentionally excluded from recorded metrics.
...
class TrajectoryPredictionModel(LightningModule):
    ...
    def setup(self, stage: str) -> None:
        if stage == "fit":
            print("[traj] Mode-based sample weighting disabled.")
...
class CombinedTrajPredSystem(LightningModule):
            ...
            if valid.any():
                ...
                if cand_idx.numel() > 0:
                    ...
                    for b in cand_idx.tolist():
                        ...
                        nn_rel = None
                        nn_score = None
                        if getattr(self.eparams, 'plot_retrieval_neighbours', True) \
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0) -> None:
        ...
        sel_mode = getattr(self.eparams, 'selection_mode', 'oracle')
