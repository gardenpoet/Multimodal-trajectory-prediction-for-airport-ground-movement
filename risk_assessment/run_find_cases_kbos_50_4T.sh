#!/bin/bash
# KBOS H50, 4T checkpoint -- find_cases_two_stage.py's mode x candidate risk
# aggregation using the trained scorer's per-candidate probabilities. Same
# checkpoint/score-head config as
# AmeliaTF_main_two_phases_4T/eval_scorer_kbos_4T_test.sh.
#
...
#   sbatch --export=ALL,LIMIT=5 risk_assessment/run_find_cases_kbos_50_4T.sh

#SBATCH --job-name=amelia_risk_kbos_50_4T
#SBATCH --output=risk_kbos_50_4T_%j.out
#SBATCH --error=risk_kbos_50_4T_%j.err
...
OUT_CSV="/gpfs/scratch/exy064/ljx/Risk-Assessment/out/risk_assessment/kbos_50_4T_cases.csv"
