#!/bin/bash
# KMSY H50, 2T checkpoint -- find_cases_two_stage.py's mode x candidate risk
# aggregation using the trained scorer's per-candidate probabilities. Same
# checkpoint/score-head config as
# AmeliaTF_main_two_phases_4T/eval_scorer_kmsy_2T_test.sh.
#
...
#   sbatch --export=ALL,LIMIT=5 risk_assessment/run_find_cases_kmsy_50_2T.sh

#SBATCH --job-name=amelia_risk_kmsy_50_2T
#SBATCH --output=risk_kmsy_50_2T_%j.out
#SBATCH --error=risk_kmsy_50_2T_%j.err
...
OUT_CSV="/gpfs/scratch/exy064/ljx/Risk-Assessment/out/risk_assessment/kmsy_50_2T_cases.csv"
