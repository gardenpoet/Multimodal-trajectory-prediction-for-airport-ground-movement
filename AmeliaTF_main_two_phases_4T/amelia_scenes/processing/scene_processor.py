...
class SceneProcessor:
    ...
    def __init__(self, config: EasyDict) -> None:
        ...
        # ------------------------------------------------------------------
        self.use_precomputed_thresholds = getattr(config, 'use_precomputed_thresholds', True)
    ...
    def process_file(self, f: str):
        ...
        Returns
        -------
        (blacklist, n_runway_skipped, n_agents_seen) or None
        """
        ...
        if num_sequences < 1:
            ...
            return blacklist, f_rw_skipped, f_agents_seen
        ...
        for i in range(0, num_sequences * self.skip + 1, self.skip):
            ...
            # process_seq now also reports runway-skip statistics
            n_skip, n_seen = result[-2], result[-1]
            f_rw_skipped  += n_skip
    ...
    def process_seq(self, frame_data, frames, seq_idx, airport_id):
        ...
        if math.isclose(seq_data[:, G.RAW_IDX.Speed].sum(), 0):
            return none_outs + (n_rw_skipped, n_agents_seen)
        if not np.isin(
