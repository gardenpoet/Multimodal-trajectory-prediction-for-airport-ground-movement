#!/bin/bash
# Trains a learned trajectory-selection scorer for KLAX's 20s-horizon 4T
...
# 27959063 failed without it, on the 2T variant of this script): gmm.py's
# attention score head reads config.decoder.pred_len to size its "full"
# key encoder at init time, defaulting to 50 (coincidentally correct for
# the 50s scripts) unless explicitly overridden here.
#
...
#SBATCH --job-name=amelia_eval_scorer_klax20_4T
#SBATCH --output=eval_scorer_klax_20_4T_%j.out
#SBATCH --error=eval_scorer_klax_20_4T_%j.err
