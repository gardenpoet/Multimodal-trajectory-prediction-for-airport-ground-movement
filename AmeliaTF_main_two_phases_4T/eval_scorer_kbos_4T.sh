#!/bin/bash
# Trains a learned trajectory-selection scorer for KBOS's 4T (num_hypotheses=4,
...
#
# Mirrors eval_scorer_kmsy_4T.sh. Unlike KMSY, the KMSY 4-hour time limit
# wasn't enough to finish training + test in one job and needed a separate
# score_test rerun; this uses a 12-hour limit instead so both stages finish
# here and no companion _test.sh script is needed.
#
...
#SBATCH --job-name=amelia_eval_scorer_kbos_4T
#SBATCH --output=eval_scorer_kbos_4T_%j.out
#SBATCH --error=eval_scorer_kbos_4T_%j.err
