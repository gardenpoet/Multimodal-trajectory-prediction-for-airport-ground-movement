#!/bin/bash
# Test-only rerun of the KMSY 4T scorer, mirroring eval_scorer_kmsy_2T_test.sh:
# use amelia_tf.eval_two_stage's "score_test" stage (_stage_score_test) to
# reload the pristine 4T backbone plus the already-saved score head and run
# test only, rather than redoing training. Submit this if
# eval_scorer_kmsy_4T.sh's test phase is killed by the time limit before
# finishing (K=4 means more per-mode iteration during test than the 2T run,
# whose test alone took ~2 hours, so this is at least as likely to happen).
#
# Needs the same decoder overrides as eval_scorer_kmsy_4T.sh (enable_score_head/
...
#SBATCH --error=eval_scorer_kmsy_4T_test_%j.err
