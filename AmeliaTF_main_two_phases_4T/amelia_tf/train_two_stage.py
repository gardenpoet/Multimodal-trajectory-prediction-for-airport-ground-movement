...
class TwoStageTrainer:
    ...
    def load_pretrained_mode_model(self, checkpoint_path: Path) -> None:
        """
        Load a pretrained mode prediction model checkpoint and verify it exists.
        This function is called when skip_mode_training=True.
        """
        log.info("=" * 70)
        log.info("Skipping Mode Model Training - Loading Pretrained Model")
        log.info("=" * 70)
        
        if not checkpoint_path.exists():
            raise FileNotFoundError(f"Mode model checkpoint not found: {checkpoint_path}")
        
        self.mode_ckpt_path = checkpoint_path
        log.info(f"Pretrained mode model loaded from: {self.mode_ckpt_path}")
        
        # Verify the checkpoint is valid by trying to load it
        try:
            checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
            if 'state_dict' not in checkpoint:
                log.warning("Checkpoint does not contain 'state_dict' key, but will try to use it anyway")
        ...
        log.info("Mode model loaded successfully, proceeding to Stage 2...")
    ...
    def run_end_to_end_test(self, datamodule: LightningDataModule) -> Dict[str, Any]:
        ...
        Uses the mode model (either trained or loaded) and the trained trajectory model.
        """
        log.info("=" * 70)
        ...
        if self.mode_ckpt_path is None:
            raise ValueError("Mode model checkpoint not available. Please train or load a mode model first.")
        if self.traj_ckpt_path is None:
            raise ValueError("Trajectory model checkpoint not available. Please train the trajectory model first.")

        log.info(f"Using mode model from: {self.mode_ckpt_path}")
        log.info(f"Using trajectory model from: {self.traj_ckpt_path}")
        ...
        # Setup logger and trainer for testing
        logger: List[Logger] = utils.instantiate_loggers(self.cfg.get("logger"))
        ...
        # Run test
        log.info("Running end-to-end test...")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # Check if we should skip mode model training and load pretrained
        if self.cfg.get("skip_mode_training", True):
        ...
        # Stage 2: Train trajectory model (always done)
        self.train_trajectory_model(datamodule)

        # Stage 3: Optional end-to-end testing
        results = {}
        if self.cfg.get("run_end_to_end_test", True):
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="train_two_stage")
