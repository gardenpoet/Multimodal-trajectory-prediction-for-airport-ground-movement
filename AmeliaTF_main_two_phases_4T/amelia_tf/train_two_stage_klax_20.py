"""
Two-stage training pipeline for aircraft trajectory prediction (KLAX, 20s horizon).

Identical to train_two_stage_klax.py; only @hydra.main's config_name differs
(train_two_stage_klax_20, which composes data: klax2.yaml -> 20s horizon,
vs the 50s config which composes data: klax.yaml).
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="train_two_stage_klax_20")
