"""
Two-stage training pipeline for aircraft trajectory prediction (KMSY, 20s horizon).

Identical to train_two_stage_kmsy.py; only @hydra.main's config_name differs
(train_two_stage_kmsy_20, which composes data: kmsy2.yaml -> 20s horizon,
vs the 50s config which composes data: kmsy.yaml). Supersedes the older
ad-hoc amelia_tf/train_two_stage_20.py, which hardcoded KMSY and referenced
the nonexistent configs/paths/default4.yaml.
