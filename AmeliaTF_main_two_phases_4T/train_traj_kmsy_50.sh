...
#SBATCH --output=traj_kmsy_50_%j.out
#SBATCH --error=traj_kmsy_50_%j.err
...
# Requires train_mode_kmsy_50.sh to have completed first (produces the fixed
# mode checkpoint this run reuses via skip_mode_training=true). Submit 3x:
#   sbatch --export=ALL,NUM_FUTURES=1 train_traj_kmsy_50.sh
#   sbatch --export=ALL,NUM_FUTURES=2 train_traj_kmsy_50.sh
#   sbatch --export=ALL,NUM_FUTURES=4 train_traj_kmsy_50.sh
...
python -m amelia_tf.train_two_stage_kmsy \
