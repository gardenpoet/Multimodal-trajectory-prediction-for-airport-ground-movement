#!/bin/bash
# Trains a learned trajectory-selection scorer for KBOS's 20s-horizon 2T
...
#
# 20s-horizon checkpoints live under a SEPARATE ckpt namespace (kbos_20, not
# kbos2) with its own data config: data=kbos2.yaml pulls in configs/data/
# default2.yaml (pred_lens=[10,20], traj_len=30), vs the 50s scripts' plain
# kbos.yaml (default.yaml, pred_lens=[20,50]). The base configs/eval_two_
...
# scenes (proc_full_scenes/, 60-length windows). data=kbos2.yaml alone
...
#SBATCH --job-name=amelia_eval_scorer_kbos20_2T
#SBATCH --output=eval_scorer_kbos_20_2T_%j.out
#SBATCH --error=eval_scorer_kbos_20_2T_%j.err
