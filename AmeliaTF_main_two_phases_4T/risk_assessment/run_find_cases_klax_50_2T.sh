#!/bin/bash
# KLAX H50, 2T checkpoint -- generalized find_cases.py test (mode x candidate
# risk aggregation using the trained scorer's per-candidate probabilities).
# Same checkpoint/score-head config as eval_scorer_klax_2T_test.sh.
#
# Sanity-check first with a few batches:
#   sbatch --export=ALL,LIMIT=5 risk_assessment/run_find_cases_klax_50_2T.sh
# Then the full run:
#   sbatch risk_assessment/run_find_cases_klax_50_2T.sh

#SBATCH --job-name=amelia_risk_klax_50_2T
#SBATCH --output=risk_klax_50_2T_%j.out
#SBATCH --error=risk_klax_50_2T_%j.err
...
OUT_CSV="/gpfs/scratch/exy064/ljx/Risk-Assessment/AmeliaTF_main_two_phases_4T/out/risk_assessment/klax_50_2T_cases.csv"
