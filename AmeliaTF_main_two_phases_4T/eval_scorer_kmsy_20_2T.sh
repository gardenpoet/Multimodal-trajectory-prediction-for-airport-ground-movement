#!/bin/bash
# Trains a learned trajectory-selection scorer for KMSY's 20s-horizon 2T
...
#
# 20s-horizon checkpoints live under a SEPARATE ckpt namespace (kmsy_20, not
# kmsy2) with its own data config: data=kmsy2.yaml pulls in configs/data/
# default2.yaml (pred_lens=[10,20], traj_len=30), vs the 50s scripts' plain
# kmsy.yaml (default.yaml, pred_lens=[20,50]). The base configs/eval_two_
...
#
# paths=default2.yaml is ALSO required (job 27892446 failed without it):
# the base eval_two_stage.yaml defaults to paths=default.yaml, whose
# scenes_dir points at the 50s preprocessed scenes (proc_full_scenes/,
# 60-length windows). data=kmsy2.yaml alone correctly sets traj_len=30 in
# the model config (so encoder_config.T_size=30), but without also
# overriding paths, the datamodule still loads the wrong (60-length) scene
# files from disk, so the model errors with "Sequence length 60 exceeds
# maximum block size 30" -- paths=default2.yaml points scenes_dir at
# proc_full_scenes2/, the actual 20s-horizon preprocessed data.
#
...
# (getattr(config, "pred_len", getattr(config, "T_pred", 50))) -- not
# declared under configs/model/combined_traj_pred.yaml's decoder: block,
# so it silently defaults to 50 (coincidentally correct for the 50s
# scripts) unless explicitly overridden here.
#
...
#SBATCH --job-name=amelia_eval_scorer_kmsy20_2T
#SBATCH --output=eval_scorer_kmsy_20_2T_%j.out
#SBATCH --error=eval_scorer_kmsy_20_2T_%j.err
