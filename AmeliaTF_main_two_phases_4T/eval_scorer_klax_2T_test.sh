#!/bin/bash
# Test-only rerun of the KLAX 2T scorer, to pick up the corrected NLL
...
# score head is already trained and saved by eval_scorer_klax_2T.sh, this
...
#
# Needs the same decoder overrides as eval_scorer_klax_2T.sh (enable_score_head/
...
#   ${ckpt_dir}/Single-Airport/klax2/traj_model/klax2_twophases_2_50.ckpt
# and the saved scorer head at:
#   /gpfs/scratch/exy064/ljx/Risk-Assessment/AmeliaTF_main_two_phases_4T/out/klax2/per_mode_scorer_hard_2T.pt

#SBATCH --job-name=amelia_eval_scorer_klax_2T_test
#SBATCH --output=eval_scorer_klax_2T_test_%j.out
#SBATCH --error=eval_scorer_klax_2T_test_%j.err
