...
class AmeliaTF(nn.Module):
    ...
    def __init__(self, config: EasyDict) -> None:
        ...
        # fix initial speed
        self.delta_v0_mlp = nn.Sequential(
            nn.Linear(self.embed_size, self.embed_size),
            nn.ReLU(),
            nn.Linear(self.embed_size, 3 * self.decoder_config.num_futures)  
        )
    ...
    def forward(self, x: torch.tensor, **kwargs) -> Tuple:
        ...
        # decoding
        pred_scores, accel_mu, accel_sigma = self.decoder_head(x)
        

        return pred_scores, accel_mu, accel_sigma
